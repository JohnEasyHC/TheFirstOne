    ...
    public function registracija(){
        ...
        if(empty($ime)){        
        	...
        	if (!preg_match("/^[a-zA-Z ]*$/",$ime)){
            	$errors['ime'] = 'Za ime koristite iskljucivo slova i prazan prostor'; 
        	}
    	}    
        ...
        if(empty($prezime)){        
        	...
        	if (!preg_match("/^[a-zA-Z ]*$/",$prezime)) {
            	$errors['prezime'] = 'Za prezime koristite iskljucivo slova i prazan prostor'; 
        	}
    	}       
        ...
        if(empty($username)){
            ...
            $errors['username']='Morate popuniti username.';
         	...
         	if (preg_match('/^[A-Za-z0-9_-]*$/', $username)) {
				...
				$errors['username']='Upotrebli ste nedozvoljene karaktere';
         }
         } 
         ...
         if(empty($password)){        
            ...
            elseif (strlen($password) <= '8') {
            	$errors['password'] = "Password mora imati najmanje 8 karaktera!";
        	}
    		...
    		if(count($errors)==0){
                ...
                $dao=new DAO();
                $dao->registrcija($ime, $prezime, $email, $username, $password);
